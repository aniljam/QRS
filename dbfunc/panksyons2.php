...
$db = new MysqliDb('localhost','genyo','diwa2012','db_qrs');
...
class LoggedAccount{
		...
		public $uID, $Pname, $QFName, $QLName, $QMName, $fullname, $Dept, $contact, $email, $empid, $roledesc, $roleid, $userstatus, $Ugen, $QRarrayID, $QRarray;
...
function addUserRole(){
	...
	$insUserArr = Array('firstname'=>$uFName, 'lastname'=>$uLName, 'middlename'=> $uMName, 'nickname'=>$uNickName, 'empid'=>$uEmpID, 'deptid'=>$uDept, 'contact'=>$uContact, 'email'=>$uEmail, 'gender'=>$uGender, 'fullname'=>$uFullname,'createdby'=>$_SESSION['uID'],'createddate'=>$datenow);
	...
	$insUserID = $db->getInsertId();
				...
				if ($db->getLastErrno() === 0){
					$insUserRoleArr = Array('userid'=>$insUserID, 'roleid'=> $uRoleID, 'userrolestat'=> 1,'createdby'=>$_SESSION['uID'],'createddate'=>$datenow);
...
function editUserRoles($id, $usRoleID){
	...
	global $db, $datenow;
      ...
      $UsDataArr = Array('empid'=>$_POST['EmpNumber'],'lastname'=>$_POST['LName'],'firstname'=>$_POST['FName'],'middlename'=>$_POST['MName'], 'fullname'=>$_POST['FName']." ".$_POST['MName']." ".$_POST['LName'],'email'=>$_POST['emailadd'],'nickname'=>$_POST['PName'],'gender'=>$_POST['Ugender'],'deptid'=>$_POST['DeptName'],'contact'=>$_POST['CNumber'],'editedby'=>$_SESSION['uID'],'editeddate'=>$datenow);
	...
	if ($usRoleID) {
      ...
      $UpdateRole = $db->update("tbl_userroles", Array('roleid'=>$_POST['rolename'], 'userrolestat'=>$_POST['userrolestat'],'editedby'=>$_SESSION['uID'],'editeddate'=>$datenow));
	}
...
function editSC(){
	...
	$arrUp = Array('subcatdesc'=>$subcatname,'subcatstatus'=>$subcatStatus);
	
	$db->where('tbl_subcategory.subcatid',$subcatID);
	$upd = $db->update('tbl_subcategory',$arrUp);
	
	$arrUp2 = Array('userid'=>$subcatAssID);
...
function AddSC($catid){
	if($catid){
		global $db;
			...
			$insSC = $db->insert('tbl_subcategory',Array('subcatdesc'=>$subcatname,'subcatStatus'=>$subcatStatus,'catid'=>$catid));
			$insID = $db->getInsertId();
			if($insID){
				$insAss = $db->insert('tbl_assignedref',Array('subcatid'=>$insID,'userid'=>$subcatAssID));
			}
				if($db->getLastErrno() === 0)
				{
...
function editCat($catid){
	if($catid){
		...
		global $db;
		 $db->where('catid',$catid);
		 $updCat = $db->update('tbl_category',Array('catdesc'=>$catname,'catstatus'=>$catstat));
		 		if($db->getLastErrno() === 0)
				{
...
function addCat(){
		...
		$catstat = $_POST['catstat'];
		global $db;
		 $insCat = $db->insert('tbl_category',Array('catdesc'=>$catname,'catstatus'=>$catstat));
		 		if($db->getLastErrno() === 0)
				{
...
function createTrans(){
		...
		$transdate = date("Y-m-d G:i:s");/*datetime format for mysql datetime datatype*/
		
		global $db;
		$insData = Array('transdatetime'=>$transdate, 'transby'=>$trnsby, 'transservtypeid'=>$typeval, 'transcat'=>$cat, 'transsubcat'=>$subcat,'transstat'=>'1', 'transdesc'=>$tdesc, 'transassignto'=>$AssignToID);
		...
		$InsId = $db->getInsertId();
				...
				if ($db->getLastErrno() === 0){
					$emailFunc = eMailer($InsId, $typeName, 1, $trnsby, $tdesc, $cat, $subcat, $transdate, $AssignToID, "");
...
function Response(){
	...
	$resInsData = Array('respdatetime'=>$respDate, 'resptransid'=>$trnsID, 'respby'=>$_SESSION['portal_loguser'], 'respactual'=>$actualResp, 'respstatus'=>$newStat, 'relativeid'=>$_SESSION['uID']);
...
function attachFileTrns($refid, $type, $relid){
	...
	if(isset($refid)){
		...
		$attachName = $_FILES['trnsAttach']['name'];
			...
			$AttachData = Array('attachpathname' => $location, 'attachtype' => $type, 'referenceid' => $refid, 'attachdatetime' => $Attachdate,'relativeid'=> $relid);
...
/*required parameters: $trnsFilter and $myRoles*/
function checkMyTransCount($trnsFilter, Array $myRoles, $type)
{
	global $db;
	...
	//$db->join("tbl_assignedref","tbl_transactions.transsubcat = tbl_assignedref.subcatid","LEFT");
